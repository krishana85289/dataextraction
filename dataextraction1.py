...
def write_data_to_excel(data, output_file_path):
    ...
    headers = [
        'InvoiceNumber', 'OrderNumber', 'InvoiceDate', 'DueDate', 'Subtotal', 
        'Tax', 'TaxRate', 'OrderTotal', 'BillingName', 'ShippingName', 
        'BillingAddress', 'ShippingAddress', 'CompanyName', 'CompanyAddress', 
        'CompanyEmail', 'CompanyPhone', 'ProductName', 'ProductSKU', 
        'ProductPrice', 'ProductQuantity', 'ProductSubtotal', 'DeliveryDate'
    ]
    ...
    for entry in data:
        ...
        product_data = entry['Products information']
        for product in product_data:
            # Merging price, and quantity, converting to float and int since products have strings
            product_price = float(product['Price'].replace(',', ''))
            product_quantity = int(product['Quantity'])
            product_subtotal = float(product['Subtotal'].replace(',', ''))

            # Add values to running totals
            price_total += product_price
            quantity_total += product_quantity
            subtotal_total += product_subtotal

            # Write product-specific information followed by common data.
            worksheet.write(row_num, len(common_data), product['Name'])
            worksheet.write(row_num, len(common_data) + 1, product['SKU'])
            worksheet.write(row_num, len(common_data) + 2, product_price)
            worksheet.write(row_num, len(common_data) + 3, product_quantity)
            worksheet.write(row_num, len(common_data) + 4, product_subtotal)
            # Safely accessing 'delivery date' with a default of None
            delivery_date = product.get('delivery date', None)
            worksheet.write(row_num, len(common_data) + 5, delivery_date)

            # Write the common data only if it's the first product in the list
            if product is product_data[0]:
                for col_num, value in enumerate(common_data):
                    worksheet.write(row_num, col_num, value)

            row_num += 1

    # Writing Totals Labels
    worksheet.write(row_num, headers.index('ProductPrice'), 'Total Price')
    worksheet.write(row_num, headers.index('ProductQuantity'), 'Total Quantity')
    worksheet.write(row_num, headers.index('ProductSubtotal'), 'Total Subtotal')

    # Writing Actual Totals
    worksheet.write(row_num + 1, headers.index('ProductPrice'), price_total)
    worksheet.write(row_num + 1, headers.index('ProductQuantity'), quantity_total)
    worksheet.write(row_num + 1, headers.index('ProductSubtotal'), subtotal_total)

    # Close the workbook to write to file.
    workbook.close()



output_path = 'D:\\Office work\\dataextraction\\output.xlsx'
...
def extract_text_from_img():
    ...
    try:
        for file in files:
            ...
            write_data_to_excel(aggregated_data, output_path)
            print(data)
            absolute_path = os.path.join(os.getcwd(), output_path)

        return send_file(absolute_path, as_attachment=True),200
